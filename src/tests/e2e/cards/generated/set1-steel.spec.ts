...
test.describe('Batch: Set 1 Steel', () => {
    ...
    test('Te Kā - Heartless - Basic Play', async ({ gamePage }) => {
        await gamePage.loadTestGame();
        ...
        if (debugAvailable) {
            // Setup: Add card to hand and enough ink
            await gamePage.addCardToHand('Te Kā - Heartless', 1);
        ...
        // Play the card
        await gamePage.clickCardInHand('Te Kā - Heartless');
        await gamePage.clickAction('Play Card');
        ...
        while (modalLoop < 5) {
            try {
               ...
               if (isBlocked || hasButton) {
                   const allBtns = await gamePage.page.locator('button').allInnerTexts().catch(() => []);
                   console.log(`Interaction needed (Step ${modalLoop + 1}) for Te Kā - Heartless. Blocked: ${isBlocked}, Button: ${hasButton}`);
                   console.log('All Visible Buttons:', allBtns);
